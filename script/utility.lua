...
function Auxiliary.RegisterRules()
	...
	local ex=Effect.GlobalEffect()
	ex:SetType(EFFECT_TYPE_FIELD)
	ex:SetCode(EFFECT_NO_EFFECT_DAMAGE)
	ex:SetTargetRange(1,1)
	ex:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	Duel.RegisterEffect(ex,0)
	local ex=Effect.GlobalEffect()
	ex:SetType(EFFECT_TYPE_FIELD)
	ex:SetCode(EFFECT_ATTACK_COST)
	ex:SetTargetRange(1,1)
	ex:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	ex:SetCost(function(e,c,tp)
		e:SetLabelObject(c)
	...
	local ex=Effect.GlobalEffect()
	ex:SetType(EFFECT_TYPE_FIELD)
	ex:SetCode(EFFECT_DEFENSE_ATTACK)
	ex:SetTargetRange(LOCATION_MZONE,LOCATION_MZONE)
	ex:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE)
	Duel.RegisterEffect(ex,0)
	local ex=Effect.GlobalEffect()
	ex:SetType(EFFECT_TYPE_FIELD)
	ex:SetCode(EFFECT_CANNOT_ATTACK_ANNOUNCE)
	ex:SetTargetRange(LOCATION_MZONE,LOCATION_MZONE)
	ex:SetTarget(function(e,c)
		if c:IsType(TYPE_CONTINUOUS) then return true end
		if not c:IsHasEffect(EFFECT_QUICK_ACTION) and Duel.GetTurnCount()==e:GetHandler():GetTurnID() then return true end
		if c:IsHasEffect(EFFECT_SLOW_ACTION) and Duel.GetTurnCount()<e:GetHandler():GetTurnID()+4 then return true end
		return false
	end)
	ex:SetProperty(EFFECT_FLAG_IGNORE_IMMUNE)
	Duel.RegisterEffect(ex,0)
